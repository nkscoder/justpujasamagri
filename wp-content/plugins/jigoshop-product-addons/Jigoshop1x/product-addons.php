...
function init_jigoshop_product_addons()
{
    ...
    class jigoshop_product_addons
    {
        ...
        {
            add_meta_box('jigoshop-product-addons', __('Product Add-ons', 'jigoshop_product_addons'),
                array(&$this, 'meta_box'), 'product', 'side', 'default');
        }

        function meta_box_css()
        {

            wp_enqueue_style('jigoshop_product_addons_css',
                plugins_url(basename(dirname(__FILE__))) . '/css/admin.css');
        }
        ...
        function meta_box($post)
        {
            ...
            <div id="product_addons" class="panel">
                    ...
                    if (is_array($product_addons) && sizeof($product_addons) > 0) {
                        foreach ($product_addons as $addon) :
                            ...
                            <p class="addon_name">
                                ...
                                <input type="text" name="addon_name[<?php echo $loop; ?>]"
                                       placeholder="<?php _e('Name', 'jigoshop_product_addons'); ?>"
                                       value="<?php echo esc_attr($addon['name']); ?>"/>
                                <input type="hidden" name="addon_position[<?php echo $loop; ?>]" class="addon_position"
                                       value="<?php echo $loop; ?>"/>
                            </p>
                            <p class="addon_type">
                                <label class="hidden"><?php _e('Type', 'jigoshop_product_addons'); ?>:</label>
                                <select name="addon_type[<?php echo $loop; ?>]">
                                    <option <?php selected('checkbox', $addon['type']); ?>
                                        value="checkbox"><?php _e('Checkboxes', 'jigoshop_product_addons'); ?></option>
                                    <option <?php selected('select', $addon['type']); ?>
                                        value="select"><?php _e('Select box', 'jigoshop_product_addons'); ?></option>
                                    <option <?php selected('radio', $addon['type']); ?>
                                        value="radio"><?php _e('Radio list', 'jigoshop_product_addons'); ?></option>
                                    <option <?php selected('custom', $addon['type']); ?>
                                        value="custom"><?php _e('Customer Input boxes',
                                            'jigoshop_product_addons'); ?></option>
                                    <option <?php selected('textarea', $addon['type']); ?>
                                        value="textarea"><?php _e('Textareas', 'jigoshop_product_addons'); ?></option>
                                    <option <?php selected('datepicker', $addon['type']); ?>
                                        value="datepicker"><?php _e('Datepickers',
                                            'jigoshop_product_addons'); ?></option>
                                    <option <?php selected('file_upload', $addon['type']); ?>
                                        value="file_upload"><?php _e('File upload',
                                            'jigoshop_product_addons'); ?></option>
                            ...
                            <p class="addon_description">
                                ...
                                <input type="text" name="addon_description[<?php echo $loop; ?>]"
                                       placeholder="<?php _e('Description', 'jigoshop_product_addons'); ?>"
                                       value="<?php echo esc_attr($addon['description']); ?>"/>
                            ...
                            <table cellpadding="0" cellspacing="0" class="jigoshop_addon_options">
                                ...
                                <tr>
                                    ...
                                    <th width="1%" class="actions">
                                        <button type="button" class="add_addon_option button"><?php _e('Add',
                                                'jigoshop_product_addons'); ?></button>
                                    </th>
                                </tr>
                                </thead>
                                <tbody>
                                ...
                                foreach ($addon['options'] as $option) :
                                    ...
                                    <tr>
                                        <td>
                                            <?php do_action('jigoshop_product_addons_before_option_label', $addon,
                                                $option, $loop) ?>
                                            <input type="text" class="addon_option_label"
                                                   name="addon_option_label[<?php echo $loop; ?>][]"
                                                   value="<?php echo esc_attr($option['label']) ?>"
                                                   placeholder="<?php _e('Label', 'jigoshop_product_addons'); ?>"/>
                                            <?php do_action('jigoshop_product_addons_after_option_label', $addon,
                                                $option, $loop) ?>
                                        </td>
                                        <td><input type="text" name="addon_option_price[<?php echo $loop; ?>][]"
                                                   value="<?php echo esc_attr($option['price']) ?>" placeholder="0.00"/>
                                        </td>
                                        <td class="actions">
                            ...
                            <a href="#" class="delete_addon"><?php _e('Delete add-on',
                                    'jigoshop_product_addons'); ?></a>
                            </div><?php
                ...
                <h4><a href="#" class="add_new_addon">+ <?php _e('Add New Product Add-on',
                            'jigoshop_product_addons'); ?></a></h4>
        ...
        function meta_box_js()
        {
            ...
            jQuery('.jigoshop_addons').append(''+
            '<div class="jigoshop_addon">'+
                '<p class="addon_name">'+
                    '<label class="hidden"><?php _e('Name', 'jigoshop_product_addons'); ?>:</label>'+
                    '<input type="text" name="addon_name[' + loop + ']"'+
                           'placeholder="<?php _e('Name', 'jigoshop_product_addons'); ?>"/>'+
                    '<input type="hidden" name="addon_position[' + loop + ']" class="addon_position"'+
                           'value="' + loop + '"/>'+
                '</p>'+
                '<p class="addon_type">'+
                    '<label class="hidden"><?php _e('Type', 'jigoshop_product_addons'); ?>:</label>'+
                ...
                '<p class="addon_description">'+
                    '<label class="hidden"><?php _e('Description', 'jigoshop_product_addons'); ?>:</label>'+
                    '<input type="text" name="addon_description[' + loop + ']"'+
                           'placeholder="<?php _e('Description', 'jigoshop_product_addons'); ?>"/>'+
                ...
                '<table cellpadding="0" cellspacing="0" class="jigoshop_addon_options">'+
                    ...
                    '<tr>'+
                        ...
                        '<th><?php _e('Price', 'jigoshop_product_addons'); ?>:</th>'+
                        '<th width="1%" class="actions">'+
                            '<button type="button" class="add_addon_option button"><?php _e('Add', 'jigoshop_product_addons'); ?></button>'+
                        '</th>'+
                    '</tr>'+
                    '</thead>'+
                    '<tbody>'+
                    '<tr>'+
                        '<td>'+
                            ...
                            '<input type="text" name="addon_option_label[' + loop + '][]" value="<?php ?>"'+
                                   'placeholder="<?php _e('Label', 'jigoshop_product_addons'); ?>"/>'+
                            '<?php do_action('jigoshop_product_addons_after_option_label_js') ?>'+
                        '</td>'+
                        '<td><input type="text" name="addon_option_price[' + loop + '][]" value="<?php ?>"'+
                                   'placeholder="0.00"/></td>'+
                        '<td class="actions">'+
                            '<button type="button" class="remove_addon_option button">x</button>'+
                        '</td>'+
            ...
            jQuery(this).closest('.jigoshop_addon_options').find('tbody').append(''+
            '<tr>'+
                '<td>'+
                    '<?php do_action('jigoshop_product_addons_before_option_label_js') ?>'+
                    '<input type="text" name="addon_option_label[' + loop + '][]"'+
                           'placeholder="<?php _e('Label', 'jigoshop_product_addons'); ?>"/>'+
                    '<?php do_action('jigoshop_product_addons_after_option_label_js') ?>'+
                '</td>'+
            ...
            var answer = confirm('<?php _e('Are you sure you want delete this add-on option?',
            'jigoshop_product_addons'); ?>');
            ...
            var answer = confirm('<?php _e('Are you sure you want delete this add-on?',
            'jigoshop_product_addons'); ?>');
        ...
        function floatvalue($value)
        {
            ...
            return floatval(preg_replace('#^([-]*[0-9\.,\' ]+?)((\.|,){1}([0-9-]{1,2}))*$#e',
                "str_replace(array('.', ',', \"'\", ' '), '', '\\1') . '.\\4'", $value));
        }
        ...
        function process_meta_box($post_id, $post)
        {
            ...
            if (isset($_POST['addon_name'])) :
                ...
                for ($i = 0; $i < sizeof($addon_name); $i++) :
                    ...
                    if (!isset($addon_name[$i]) || trim($addon_name[$i]) == '') {
                        continue;
                    }
        ...
        function product_addons()
        {
            ...
            $addonPrices['product_price'] = strip_tags(jigoshop_price($product->get_price(),
                array('with_currency' => false)));
            ...
            if (is_array($product_addons) && sizeof($product_addons) > 0) {
                foreach ($product_addons as $addon) :
                    ...
                    ?>
                    <div class="<?php echo apply_filters('product_addons_addon_css_class', 'product-addon',
                        $addon) ?> product-addon-<?php echo sanitize_title($addon['name']); ?>">
                        <?php if ($addon['name']) : ?>
                            <h3><?php echo wptexturize($addon['name']); ?><?php if ($addon['type'] == 'file_upload') {
                                echo sprintf(__('(max size %s)', 'jigoshop_product_addons'), $this->max_upload_size());
                            } ?></h3><?php endif; ?>
                        <?php if ($addon['description']) : ?>
                            <p><?php echo wptexturize($addon['description']); ?></p><?php endif; ?>
                        <?php

                        switch ($addon['type']) :
                            case "checkbox" :
                                foreach ($addon['options'] as $option) :
                                    $price_value = apply_filters('jigoshop_multi_currencies_exchange',
                                        $option['price']);
                                    ...
                                    $checked = (isset($_POST[$aname]) && in_array(sanitize_title($option['label']),
                                            $_POST[$aname])) ? ' checked="checked"' : '';
                            ...
                            case "select" :
                                $aname = 'addon-' . sanitize_title($addon['name']);
                                echo '<p class="form-row form-row-wide ' . $aname . '"><select name="' . $aname . '">' . apply_filters('product_addons_select_option_none',
                                        '<option value="">' . __('None', 'jigoshop_product_addons') . '</option>',
                                        $addon);
                                foreach ($addon['options'] as $option) :
                                    $price_value = apply_filters('jigoshop_multi_currencies_exchange',
                                        $option['price']);
                            ...
                            case "datepicker" :
                                foreach ($addon['options'] as $option) :
                                    ...
                                    $value = isset($_POST[$aname]) ? $_POST[$aname] : '';
                                    echo '<p class="form-row form-row-wide ' . $aname . '"><label>' . wptexturize($option['label']) . $price . ($show_colon ? ':' : '') . ' <input type="text" class="input-text" id="' . $datepicker_id . '" name="' . $aname . '" value="' . $value . '" placeholder="' . __('Click to select a date',
                                            'jigoshop_product_addons') . '"/></label></p>';
                                    $datepickers[$datepicker_id] = $addon;
                                    wp_enqueue_script('jquery-ui-datepicker', false, array('jquery', 'jquery-ui-core'));
                                    wp_enqueue_style('jquery.ui.theme',
                                        plugins_url('/css/datepicker/jquery-ui-1.8.22.custom.css', __FILE__));
                                endforeach;
                                break;
                            case "file_upload" :
                ...
                jQuery(document).ready(function () {
                    ...
                    function countTotalPrice() {
                        ...
                        if (totalPrice > 0) {
                            ...
                            if (jQuery('#addon_total_price').length == 0) {
                                var endPriceObj = jQuery('<div id="addon_total_price"><span><?php _e('Total price',
                                        'jigoshop_product_addons') ?>: </span><span class="price">' + priceForDisplay + '</span></div>');
                    ...
                    if (count($datepickers)) {
                        ...
                        foreach (array_keys($datepickers) as $id) {
                            $dp_settings = apply_filters('jigoshop_product_addons_datepicker_settings', $dp_settings,
                                $datepickers[$id]);
                            $pairs_arr = array();
                ...
                foreach ($product_addons as $addon) :
                    ...
                    switch ($addon['type']) :
                            ...
                            foreach ($addon['options'] as $option) :
                                ...
                                if (array_search(sanitize_title($option['label']), $posted) !== false) :
                                    ...
                                    $cart_item_meta['addons'][] = array(
                                        ...
                                        'price' => esc_attr(apply_filters('jigoshop_multi_currencies_exchange',
                                            $option['price'])),
                                        'addon' => $addon,
                                        'option' => $option
                                    );
                        ...
                        case "file_upload" :
                            ...
                            break;
                        default:
                            $cart_item_meta = apply_filters('jigoshop_product_addons_add_cart_item', $cart_item_meta,
                                $addon);
                            break;
                    endswitch;
        ...
        function get_item_data($other_data, $cart_item)
        {
            if (isset($cart_item['addons'])) {
                ...
                foreach ($cart_item['addons'] as $addon) {
                    ...
                    $valueAdd = '';
                    if ($addon['price'] > 0) {
                        $valueAdd .= ' (' . jigoshop_price($addon['price']) . ')';
                    }
                    ...
                    if ($addon['addon']['type'] == 'file_upload') {
                        ...
                        } elseif (strpos($addon['value'], 'http://') === 0 || strpos($addon['value'],
                                'https://') === 0
                        ) {
                            $prefix = $addon['option']['label'] . $valueAdd;
                            $valueDisplay = $prefix . ': <a href="' . $addon['value'] . '">' . $addon['display'] . '</a>';
        ...
        function update_order_item($item)
        {
            ...
            foreach ($this->orderItems as $key => $savedItem) {
                ...
                if ($savedItem['id'] == $item['id'] && isset($savedItem['item_meta']) && in_array($md5,
                        $_POST['item_addons_md5']) && !in_array($key, $this->checkedItems)
                ) {
                ...
                <table class="meta" cellspacing="0">
                    ...
                    if (isset($item['item_meta']) && is_array($item['item_meta']) && sizeof($item['item_meta']) > 0) :
                        foreach ($item['item_meta'] as $key => $meta) :
                            ...
                            $display_value = apply_filters('jigoshop_product_addons_order_item_meta_display_value',
                                $display_value, $meta, $item['item_meta']);
                        ...
                        if (is_dir($extensions_dir . DIRECTORY_SEPARATOR . $entry)) {
                            if ($handle_dir = opendir($extensions_dir . DIRECTORY_SEPARATOR . $entry)) {
                                while (false !== ($file = readdir($handle_dir))) {
                                    ...
                                    if (isset(${$class_name}) && is_object(${$class_name}) && method_exists(${$class_name},
                                            'setContext')
                                    ) {
                                        $obj = ${$class_name};
                                        $obj->setContext($this);
                            ...
                            $class_name = basename($file, ".class.php");
                            if ($class_name && is_object(${$class_name}) && method_exists(${$class_name},
                                    'setContext')
                            ) {
                                $obj = ${$class_name};
                                $obj->setContext($this);
                ...
                foreach ($cart['addons'] as $key => $addon) {
                    switch ($addon['addon']['type']) {
                        ...
                        case "file_upload" :
                            ...
                            $addon['price'] = esc_attr(apply_filters('jigoshop_multi_currencies_exchange',
                                $addon['option']['price']));
...
if (is_admin()) {
    add_filter('plugin_action_links_' . plugin_basename(dirname(JIGOSHOP_PRODUCT_ADDONS_DIR) . '/bootstrap.php'),
        function ($links) {
